# tiling 模块知识（解释型，模拟仓库过时文档）

> 来源：算子平台 add_custom 模板 Host 侧 Tiling 计算（文档滞后于源码，v1 版）
> 注意：本知识为解释型描述，不含源码原文。
...
4. 输出字段：
   - `totalLength`：原样透传
   - `blockNum`：第 3 步结果
   - `numPerCore`：`tilesPerCore * TILE_LENGTH`（每核处理的元素数）
   - `tailNumLastCore`：**v1 文档缺失该字段的计算说明（过时点）**，请按"最后一个 block 的实际元素数"补齐语义
