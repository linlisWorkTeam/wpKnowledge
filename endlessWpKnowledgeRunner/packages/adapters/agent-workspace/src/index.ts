...
import type { AgentWorkspaceProvider, AgentWorkspaceView } from '../../../contracts/src/index.ts';
...
export class LocalAgentWorkspace implements AgentWorkspaceProvider {
  ...
  async materialize(input: {
    ...
    for (const path of readablePaths) {
      ...
      const source = await realpath(join(sourceRoot, path));
      ...
      await writeImmutable(join(viewRoot, path), bytes);
