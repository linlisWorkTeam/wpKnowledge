...
**结论先行**：业界没有"标准答案"，但存在清晰的**谱系**：从「固定流水线 + 文件/产物交接」（MetaGPT、ChatDev、gpt-engineer、CrewAI sequential）到「图/事件驱动 + 消息总线」（LangGraph、AutoGen 0.4、OpenHands）。**『固定流水线+文件交接』有大量直接印证案例，且恰是文档生成类任务的主流做法**；反驳主要来自"灵活编排派"和"单 agent 派"，但它们的复杂度和适用场景与知识飞轮项目不同。
...
- **链接**：https://github.com/foundationagents/metagpt （论文：arxiv.org/abs/2308.00352）
...
- **链接**：https://github.com/openbmb/ChatDev （论文：arxiv.org/abs/2307.07924）
...
```text
流程图：业界开源项目编排谱系（按复杂度递增 →）
  简单 ──────────────────────────────────────────────────────► 复杂

  固定流水线+文件交接        图/状态驱动           事件/消息总线          自由对话
  ┌────────────────┐   ┌────────────────┐   ┌────────────────┐   ┌────────────────┐
  │ gpt-engineer   │   │ LangGraph      │   │ AutoGen 0.4    │   │ MetaGPT 消息池  │
  │  步骤A→B→C     │   │  (A)→(B)→(C)   │   │  Agent─►Event  │   │  Pub/Sub 订阅   │
  │  产物落盘       │   │  共享State     │   │  事件总线       │   │  角色自由协作   │
  ├────────────────┤   ├────────────────┤   ├────────────────┤   ├────────────────┤
  │ CrewAI seq     │   │ LangGraph      │   │ OpenHands      │   │ ChatDev 阶段内  │
  │  Task输出交接   │   │  supervisor    │   │  EventStream   │   │  双角色对话     │
  ├────────────────┤   │  /swarm        │   │  (可重放)       │   └────────────────┘
  │ ChatDev 阶段间  │   ├────────────────┤   └────────────────┘
  │  文件交接       │   │ AutoGen 0.2    │
  ├────────────────┤   │  GroupChat     │
  ...
  项目数：印证"流水线+交接" = 7+ 个 ｜ 印证"图/事件" = 4 个 ｜ 单agent替代 = 3 个
```
...
| Claude Code subagents | 动态委派 | 上下文隔离+回传 | 否 | ✅ 印证 |
