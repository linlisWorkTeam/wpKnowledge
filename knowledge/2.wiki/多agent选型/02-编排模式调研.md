...
- **业界代表**：CrewAI Sequential Process；AutoGen Sequential Chat；LangGraph 链式图。
...
```text
流程图：Fan-out / Fan-in（扇出扇入，map-reduce）
                    ┌──────────┐
   任务 ───────────►│  拆分子任务 │
                    └────┬─────┘
              ┌──────────┼──────────┐
...
```text
流程图：Orchestrator-Worker（编排者-工作者）
                    ┌────────────┐
   任务 ───────────►│ Orchestrator│◄── 动态规划/分派/汇总
                    └─────┬──────┘
              ┌───────────┼───────────┐
              ▼           ▼           ▼
        ┌─────────┐ ┌─────────┐ ┌─────────┐
        │ Worker1 │ │ Worker2 │ │ Worker3 │   ← 专用worker并行
        └────┬────┘ └────┬────┘ └────┬────┘
             └───────────┼───────────┘
                         ▼
                    ┌─────────┐
...
流程图：Hierarchical（分层）
              ...
              ▼          ▼          ▼
        ┌─────────┐ ┌─────────┐ ┌─────────┐
        │ Manager1│ │ Manager2│ │ Manager3│  ← 中层：分解/督导
        └────┬────┘ └────┬────┘ └────┬────┘
        ┌────┼────┐      │      ┌────┼────┐
        ▼    ▼    ▼      ▼      ▼    ▼    ▼
       W1   W2   W3    W4     W5   W6   W7   ← 底层：执行
...
- **业界代表**：MetaGPT（产品经理→架构师→工程师→QA 的 SOP 层级，arXiv:2308.00352）；LangGraph Hierarchical 文档；CrewAI Hierarchical Process。
- **决策要点**：任务规模大且**可天然分层**时选，知识飞轮场景中按代码库层级组织 agent 非常契合。
- 来源：https://arxiv.org/abs/2308.00352 ｜ https://langchain-ai.github.io/langgraph/concepts/multi_agent/
...
- **业界代表**：LangGraph Supervisor 库（langchain-ai/langgraph-supervisor）；AutoGen GroupChat（带 manager）；CrewAI manager agent。
...
- **业界代表**：**LangGraph（事实标准）**、AutoGen Graph 团队模式、Dify/Coze 工作流、Temporal（通用执行层）。
...
```text
流程图：Debate / Consensus（辩论/共识）
   ┌─────────┐  ┌─────────┐  ┌─────────┐
   │ Agent A │  │ Agent B │  │ Agent C │   ← 各自作答
   └────┬────┘  └────┬────┘  └────┬────┘
        └────────────┼────────────┘
                     ▼
              ┌───────────┐
...
- **优点**：论文实证可显著提升推理与事实性（Du et al. 2023）；暴露单 agent 盲点；无需额外训练。
- **缺点**：成本随 轮次×agent数 暴涨；可能收敛到共同错误（群体偏差）；评判 agent 自身会错；延迟高。
- **适用**：高价值低频率决策，如代码方案评审、安全审查、事实核查、评测答案仲裁。
- **业界代表**：Du et al. Multiagent Debate（arXiv:2305.14325）；Society of Minds（arXiv:2305.17066）；CrewAI Consensual Process；AutoGen 批判模式。
- **决策要点**：**质量优先于成本/延迟**且有可仲裁答案时选；用不同模型/提示词提升多样性可显著增强效果。
- 来源：https://arxiv.org/abs/2305.14325 ｜ https://composable-models.github.io/llm_debate/
...
```text
流程图：Event-driven（事件驱动）
   ┌──────────┐   发布    ┌──────────────┐   订阅    ┌──────────┐
   │ Agent 1  │ ────────► │  事件总线(EVENT)│ ◄─────── │ Agent 3  │
   └──────────┘           └──────┬───────┘           └──────────┘
                                 │
                          ┌──────▼───────┐
                          │ Agent 2（异步）│
...
```text
流程图：Evaluator-Optimizer（评估-优化器，常用作流水线内的反馈环）
   ┌──────────┐   生成   ┌──────────┐
   │ 生成器 Gen │ ──────► │ 评估器 Eval│
   └────▲─────┘          └────┬─────┘
        │  达标→输出            │ 未达标
...
- **Network（网络式）**：任意 agent 可互相通信的无结构拓扑；表达力最强但最难控制；LangGraph 的 network 架构、AutoGen 默认 GroupChat。来源：https://langchain-ai.github.io/langgraph/concepts/multi_agent/
- **AutoGen 会话模式**：two-agent chat / sequential chat / group chat / nested chat，是上述模式的对话式实现。来源：https://microsoft.github.io/autogen/0.2/docs/tutorial/conversation-patterns/
...
- AutoGen 会话模式: https://microsoft.github.io/autogen/0.2/docs/tutorial/conversation-patterns/
...
- Society of Minds: https://arxiv.org/abs/2305.17066
- MetaGPT: https://arxiv.org/abs/2308.00352
- bMAS 黑板架构 LLM 化: https://arxiv.org/abs/2507.01701
- LLM-MAS 综述: https://arxiv.org/abs/2412.17481
