...
### 4.5.3 spec 驱动代码生成（三 agent：TDD / Code / 独立检查）

```text
流程图：spec→TDD + spec→code + 独立检查（三 agent 分工）
                    ┌──────────────────┐
                    │ spec / 知识文档    │
                    └────────┬─────────┘
                             │ 同一份 spec（注意：共享盲区风险，见下方注）
              ┌──────────────┼──────────────┐
              ▼              ▼              ▼
   ┌────────────────┐ ┌────────────────┐ ┌────────────────────────┐
   │ Agent1: TDD    │ │ Agent2: Code   │ │ Agent3: 独立检查        │
   │ 生成测试/冒烟    │ │ 生成实现代码    │ │ （CCR 模式，全新 session）│
   │ 需求澄清        │ │ 只读 spec+接口  │ │ 只读：diff+判据清单      │
   ...
   注（共享盲区）：Agent1 和 Agent2 从同一份 spec 推导，共享对 spec
   的理解盲区——Agent1 的测试只验证"它理解的 spec"，代码全绿但双方
   可能共同误解。因此：
