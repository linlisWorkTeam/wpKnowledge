...
## 4.5 本项目当前 agent 架构（mvp-flywheel 实况）

> 本节按当前 mvp-flywheel 代码实况绘制（run 产物落盘于 `samples/tiling/run_llm_iterative/data/runs/<run_id>/`），是"我们实际怎么编排"的现状图。

### 4.5.1 总体架构（角色 + 编排 + 产物）

```text
流程图：知识飞轮当前 agent 架构（mvp-flywheel 实况）
                 ┌──────────────────────────────────────────────────┐
                 │          编排层 KnowledgeFlywheel（确定性状态机）  │
                 │      决策 / 回滚 / 预算 / 审计 / 断点续跑          │
                 │      （代码实现，不烧 token，非 LLM 主 agent）      │
                 └───────┬──────────────────────────┬───────────────┘
                         │                          │
   源码 src_dir          │                          │
   （只读，写保护）        │  首版：无 initial_doc 时   │
        │               ▼                          ▼
        │   ┌────────────────────┐      ┌────────────────────┐
        ├──►│ 知识生成 Agent      │      │ Coder Agent         │
        │   │ KnowledgeGenAgent  │      │ CoderAgent          │
        │   │ generate() 读源码   │      │ generate_code()     │
        │   │ revise() 改知识     │      │ 只读知识+接口头文件    │
        │   │ （唯一执笔者）        │      │ （物理看不到源码-沙箱） │
        │   └─────────┬──────────┘      └─────────┬──────────┘
        │             │ 知识文档 .md              │ 临时代码 .cpp
        │             ▼                          ▼
        │   ┌────────────────────┐      ┌────────────────────┐
        │   │  知识候选区          │      │ 评测闭环（只读程序）    │
        │   │ run_root/knowledge │      │ compile(g++ -Werror) │
        │   │ version+1/修订      │      │ + 探针测试 + 相似度    │
        │   └────────────────────┘      └─────────┬──────────┘
        │                                        │ 评测报告 .json
        │                                        ▼
        │                              ┌────────────────────┐
   ...
   写保护：src / evalset / knowledge / interfaces 全程 SHA-256 前后校验
   沙箱：Coder 只读 knowledge/+interfaces/，src_dir 硬禁止（即使恶意配置）
   产物交接：知识 md / 代码 cpp / 评测 json / 归因 json / 修订 json 全部落盘
```

### 4.5.2 每轮状态机（decide() 实况）

```text
流程图：每轮状态机（fw/revise/__init__.py::decide 实际逻辑）
                     ┌─────────────┐
                     │  进入第 N 轮   │
                     └──────┬──────┘
                            ▼
                  ...
                  └──────┬───────────┘
                         ▼
        ┌────────────────────────────────┐
        │ ZERO_CASES / 编译失败 / 无效报告? │──是──► stopped（INVALID_EVALUATION）
        └────────────────┬───────────────┘
                         ▼ 否
        ┌────────────────────────────────┐
        │ 方差 > 0.02（unstable）?         │──是──► 预算尽? stopped : unstable
        └────────────────┬───────────────┘
                         ▼ 否
        ┌────────────────────────────────┐
        │ confidence < 上一轮（倒退）?     │──是──► rollback（恢复 doc_history[-2]）
        └────────────────┬───────────────┘
                         ▼ 否
        ┌────────────────────────────────┐
        │ confidence ≥ 0.8（门禁阈值）?    │──是──► holdout 验证
        └────────────────┬───────────────┘        │
                         ▼ 否                     ├─ holdout 通过 → pass（发布知识）
        ┌────────────────────────────────┐        └─ holdout 不达标 → stopped
        │ 预算耗尽（轮次/时间）?            │──是──► stopped（MAX_BUDGET）
        └────────────────┬───────────────┘
                         ▼ 否
                   iterate（TRAIN_BELOW_THRESHOLD）
                         │
                         ▼
        归因 → 修订指令 → 知识 version+1 → 下一轮
```

### 4.5.3 产物交接链（每轮落盘，可审计/可断点续跑）

```text
流程图：产物交接链（run_root 下全部落盘）
   run_id = uuid（本次运行唯一）

   ┌─────────────┐   ┌─────────────┐   ┌─────────────┐
   │ knowledge/  │──►│    code/    │──►│    eval/    │
   │ tiling_vN.md│   │ round_r_g1  │   │ train_round │
   │ （候选+发布） │   │  .cpp       │   │  _N.json    │
   └─────────────┘   └─────────────┘   └──────┬──────┘
                                              ▼
   ┌─────────────┐   ┌─────────────┐   ┌─────────────┐
   │corrections/ │◄──│ attribution/ │◄──│ 决策+归因    │
   │ 修订指令队列  │   │ round_N.json │   │ (decide)    │
   └──────┬──────┘   └─────────────┘   └─────────────┘
          │
          ▼ 修订 → knowledge v+1 → 下一轮（回到第一格）
   ┌─────────────┐   ┌─────────────┐
   │ protection/ │   │ config.     │   result.json（汇总）
   │ before/after│   │ snapshot    │   run_id / rounds / decision
   └─────────────┘   └─────────────┘
   （SHA-256 前后校验）  （配置冻结，可复现）
```

### 4.5.4 与调研结论的对照（现状自我评估）

| 调研维度 | 当前实现 | 对照结论 |
|---|---|---|
| 编排范式 | Pipeline + Evaluator-Optimizer 反馈环（确定性状态机） | 与文档生成类任务主流（gpt-engineer/DocAgent 路线）一致 |
| 主 agent | 无 LLM orchestrator，决策全在代码 | 编码类任务并行度低，确定性编排最省（Anthropic 教训支持"克制"） |
| Review | 独立上下文（CCR），Review 只看评测报告 | 已满足 CCR 实证（F1 最高），保持即可 |
| 通信 | 文件交接（产物落盘），非对话 | 业界强印证（7+ 案例），可审计可断点续跑 |
| 写保护/沙箱 | SHA-256 前后校验 + 路径白名单 | 满足生产级护栏要求 |
| 待补强 | 分块 subagent 并行（大库）、跨模型 review、spec→TDD 冒烟 | 见 README 决策点 3/4/5 |
