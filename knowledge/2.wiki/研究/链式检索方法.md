...
输出：`/tmp/*_results.md`（Markdown 表格）+ `.json`（原始数据）。表格可直接粘贴进 候选论文池.md。
...
| 轮次 | 日期 | 方法 | 结果 |
|------|------|------|------|
| 第1轮 | 2026-08-20 | 8 篇核心滚雪球 + 7 组定向关键词 | 44 + 12 候选 → 候选论文池.md（40+ 篇） |
| 第2轮 | 2026-08-20 | 扩展核心集（12 篇）+ 14 组定向词（S2 429 限速 → 切 arXiv API）+ GitHub 高 star 检索 | 60 论文 + 64 仓库 → 候选论文池.md 扩充至 100+ 条目 + 新单篇 |
...
- **论文"最新 ≠ 最可信"**：预印本无同行评审，自改进领域普遍乐观偏差（见 反馈闭环/自进化Agent脆弱性.md）
