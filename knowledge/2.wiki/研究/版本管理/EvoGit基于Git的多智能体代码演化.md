...
偏序只覆盖了一部分版本对，**许多版本对之间根本排不出先后**。没有祖先-后代关系的两个版本就是这种情形，它们来自并发分支，是潜在的重组候选。
...
**变异**：agent 从当前前沿选一个版本作为起点，接收设计规格、当前文件结构和**随机选取的可编辑代码段**，提出局部小改动（改一个函数、插一个条件、重构几行）。约束是只能编辑单个文件内的具体区段，或新建文件插入代码片段，禁止跨文件的大规模重构。改动后做成对比较，依据构建状态、linter 诊断、类型检查和测试结果判断是否优于父版本。通过且无回归则作为后代节点加入图中，否则丢弃。
