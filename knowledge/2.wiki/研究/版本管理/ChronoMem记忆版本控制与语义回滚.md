...
关键设计：**混合检索在版本索引上进行，而不是在完整快照或原始事件上**。这让回滚选择的计算量由提交数量决定，**与快照大小无关**。快照载荷只在目标版本选定之后才被访问。
...
论文特意把两件事分开：**选择错误**和**恢复正确性**。这让评测可解释，也让恢复路径本身是确定性的、可复现的。
...
ChronoMem 的评测协议是暴露后回滚：恢复之后要重新做下游任务，验证行为是否符合预期。这**直接印证了本项目的一条已有判断**——回滚后的版本不能直接采用，必须重新评测。
...
ChronoMem 回滚会截断后续版本。本项目 `KnowledgeVersion` 的设计是**保留** `SUPERSEDED` 历史，不删除版本。这一点上应该采用本项目原有的取向（保留），而不是照搬 ChronoMem 的截断做法，因为被截断的版本可能是后续有价值的分支。
