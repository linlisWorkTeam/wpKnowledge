...
| **一句话总结** | 把新版本当作受治理的部署候选而非立即替换：四级兼容性检查加七阶段升级流水线，回滚可靠性作为准入条件而非事后补救 |
...
第四类最值得注意：**恢复能力的退化是一种独立失效模式**，不是「回滚没做好」这种工程细节。一个性能很高的候选版本，如果失败后系统无法恢复，就应该被拒绝准入。
...
恢复兼容性有一条关键判断：**它是准入判据，不是失败后才考虑的问题**。论文为此定义了「恢复就绪度」评分，得分低就限制激活模式或要求人工审查。
...
论文明确说：目标在于同时满足五项性质，而非不惜代价提高升级接受率。这五项是改进性、安全性、兼容性、可恢复性、可审计性。
...
这是本篇最值得移植的一条判断。当前 `Evaluation.md` 把回滚列为未实现的目标要求，位置是**事后补救**（迭代多轮失败后考虑回退）。
...
论文区分硬回滚、软降级、档案降级。本项目目前只有 `ROLLING_BACK` 一个状态，对应硬回滚。可以考虑的补充是**软降级**：把可疑版本转为 `LOW_CONFIDENCE` 而非直接回退。
