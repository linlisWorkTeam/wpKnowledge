...
论文证明了它与 Gödel Machine 的关键联系：在某个假设下，**拥有真实 CMP 的预言机足以实现 Gödel Machine**。这让 CMP 成为一个有理论依据的引导信号，已经超出启发式的层次。
...
HGM 每一轮在「扩张」和「评测」之间选择。扩张时对每个节点采样决定是否扩张，选出节点后做自我修改产出子节点。评测时对还有剩余任务的 agent 采样，给选中的 agent 分配一个基准任务并更新计数（同时更新其祖先的计数）。
...
- **对标人类**：用 GPT-5-mini 在 SWE-bench Verified 上优化出的 agent，在 SWE-bench Lite 上用 GPT-5 评测，达到人类水平，**与人类工程化编码 agent 的最好官方结果持平**
...
这比单纯看「连续几轮分数没提升」更有结构信息：停滞从谱系角度判断，落在单条时间线上的观察看不到这一层。
...
HGM 最本质的贡献是区分了两件事：**当前表现**和**改进潜力**。DGM 拿前者当后者的代理，论文用实验证据说明这两者相关性很弱。

这个区分对本项目有直接含义。`EvaluationReport` 衡量的是「这一版知识文档对不对」，这两个指标落在不同问题上，用它推断「还有多少改进空间」**并不可靠**。两个问题需要两套判据。

**② 用结构信息替代分数**

HGM 的判据完全基于树的拓扑结构（谁是谁的 clade）加聚合统计，不需要引入新的评分维度。EvoGit 用偏序替代标量适应度，两者都选择让结构本身承担判断职责。
...
本项目缺成本预算（`Evaluation.md` 列为未实现）。HGM 说明：**选择判据的质量本身就是一种预算效率手段**，光靠加一个计数器限制总量做不到这一点。
