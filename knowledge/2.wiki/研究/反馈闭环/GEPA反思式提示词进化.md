...
修订链路是这样的：评测出报告，评审角色产出修订意见，应用层做映射，下一轮文档生成照着改。

评审这份工作有一块还没验收，项目文档自己写了：真实模型对文档问题的定位、因果分析和历史总结质量尚未验收。

再看修订意见的结构。它只有五个字段：编号、知识路径、判断标准、证据引用、风险。其中判断标准就是一个字符串。

问题出在这里。同一轮里如果冒出几条意见指向同一段落的不同毛病，或者两条意见互相冲突，下一轮生成看到的就是几个并列的字符串。谁先谁后，谁更重要，字符串本身说不出来。GEPA 挑候选的那套办法正好补这一段。

### 解决哪个环节的什么问题

**环节：评审输出意见之后、下一轮文档生成开始之前这一段。**

现在这些意见形成一个列表，整份交给生成角色去改。麻烦在于列表内部可能打架。同一个段落，一条意见要求补充边界条件，另一条要求简化描述，两条都执行就会互相抵消。还有一种情况：某条意见只对个别测试用例有效，却被当成通用问题处理，改了不该改的地方。

### 具体怎么落地

**① 先按测试项找最佳，再看总通过率**

这个项目的测试集是固定下来的：候选测试要在参考源码上真实跑通并逐入口核对，通过后按源码内容固定，源码不变就复用。这正好是 GEPA 需要的那种样本集。

做法是把每轮产出的文档版本放到这份固定测试集上逐项跑，记下每个版本在每一项上的通过情况。关键是别只看总通过率。

评测报告里已经有测试计数和关键失败数，要补的是逐项的通过明细。有了明细才能判断某个版本在哪些项上是唯一能过的。

**② 按覆盖面决定哪条意见先执行**

设想一种情况：某个文档版本在多数测试项上表现平平，但有一项关键测试只有它过得去。那么它带来的那部分修订内容就该保住。

落地时给修订意见加一个字段，记下这条意见针对哪些测试项。下一轮生成的输入按覆盖面加权，优先执行覆盖了独有测试项的意见。

**③ 两条意见冲突时怎么选**

判断依据是哪条覆盖的测试项更多。至于哪条表述听起来更合理，这个不参与决策。这样能避免在自然语言层面扯皮。

### 提供了什么思路

**① 互补性比总分更重要**

这是 GEPA 消融给出的最硬结论。61.28 对 54.89 的差距说明：保留候选的意义在于能力互补，不在于分数接近。本项目如果引入多候选文档版本，选择依据应该是「在哪些测试项上不可替代」而非「综合评分」。
