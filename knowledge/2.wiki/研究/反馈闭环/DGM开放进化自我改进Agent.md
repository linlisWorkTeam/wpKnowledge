...
项目在 `docs/specs/domainFunction/knowledge/Knowledge.md` 里承认了一件事：

> 回滚状态仅为迁移兼容，当前不自动选择历史最优并回滚。

而代码那边其实走得比文档更远。`src/domain/Domain.ts` 里 `ROLLING_BACK` 已经是业务状态机的一个正式状态，迁移边也接好了：
...
`src/application/services/AutomatedProjectWorkflow.ts` 同样把 `ROLLING_BACK` 和 `PLANNED`、`ITERATING` 一起映射到了 `GENERATING`。

**换句话说，回滚的通道已经铺好，缺的只是「回滚到哪个版本、依据什么判断」这个决策逻辑。** DGM 的归档机制正好填这一段。
