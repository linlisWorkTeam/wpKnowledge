...
### 11.1.1 为什么是 5 个角色 → 归并成 3 个执行 agent

案例的 5 个角色按**项目阶段**拆得细（基线生成与迭代实现分属两阶段，所以 Base Developer 与 Verifier 分开）；本项目按**职责**划分最小充分集（知识生成/Coder/Review + 编排层 + 自动化评测闭环），两者分工逻辑一致、粒度不同。归并逻辑如下：

| 案例 5 角色 | 归并去向 | 归并理由 |
|------------|---------|---------|
| Base Developer | 知识生成 Agent（首版生成阶段） | 同构：从知识产出初始代码/文档，不读官方源码 |
| Comparator (A) | Review Agent（差异对比环节） | 同属"只读对比 + 出报告" |
| Landing Reviewer (R) | Review Agent（归因/落地审查环节） | 与 Comparator 同属只读审查，合并后扩展出"知识是否正确落地"的语义审查职责（见 §12.1 第 6 条） |
| Evolver (B) | 知识飞轮（决策）+ 知识生成 Agent（修订执行） | 案例 Evolver 既决策又执行；本项目坚持"谁决策谁不执笔"，拆成编排层决策 + 知识生成 Agent 执行 |
| Verifier (C) | Coder Agent（迭代执行） | 同构：按清单逐条实现，评测驱动 |
| 主调度 | 知识飞轮（编排层，状态机） | 同构：循环决策 + 校验关卡 |

**归并的三条原则：**

1. **生成与验证分离**（本项目硬原则）：案例的 Verifier 既要实现又要自我证明，本项目拆成 Coder（只写不验）+ Review（只审不写）+ 评测闭环（只测不改），杜绝"自己写自己评"
2. **评测闭环自动化**：案例由 Verifier 承担的部分评测验证工作，本项目交给纯自动化单元（编译 + 测试 + 相似度），不需要单独一个 LLM 角色
3. **3+1+1 最小充分集**：知识生成（写知识）、Coder（写代码）、Review（审）、编排层（决策）、评测闭环（测）。任何职责都能落入这五格之一，无重叠、无遗漏

**代价与注意**：Review Agent 承担了 Comparator 与 Landing Reviewer 两份职责，角色 prompt 需区分"差异对比"与"落地审查"两种模式；Evolver 拆开后，编排层必须有明确的修订指令格式（readlist 三字段，见 §12.1 第 5 条）才能让知识生成 Agent 精确执行。

### 11.2 机制对照（哪些可直接借鉴 ⭐）

| 本案例机制 | 本项目现状 | 借鉴价值 |
|-----------|-----------|---------|
| **信息隔离**：每角色独立新上下文，不共享历史会话 | 三角色分离（知识飞轮实现方案.md §3） | ✅ 更强：连"历史会话"都不共享，彻底防串扰 |
...
| **增量知识版本（标题作用域 Vn）** | 溯源链接 + 按段落迭代（知识飞轮实现方案.md §4/§5） | ✅ 同构：版本号精确到段落级 |
| **知识副本继承（knowledge_worktree）** | 防污染回滚（知识飞轮实现方案.md §5.3） | ✅ 更强：每轮复制完整副本，禁止重读原始 wiki，天然防污染 |
| **知识检索审计 wrapper**（确定性命令白名单） | 未细化 | ⭐ 新增借鉴点：检索行为可审计，防 LLM 乱检索 |
| **readlist 三字段**（ID+路径+判据） | 反馈两层结构（知识飞轮实现方案.md §5.2） | ✅ 同构：验证清单结构化，比自然语言更可执行 |
| **B1 冻结后才进 B2** | 无对应 | ⭐ 新增借鉴点：阶段产物冻结，防止后续阶段污染前序结论 |
| **失败机制总账（failed-mechanism-ledger）** | 薄弱点地图（知识飞轮实现方案.md §4.3） | ✅ 同构：失败机制积累，辅助决策 |
| **Landing Review（知识是否正确落地）** | 仅差异对比 | ⭐ 扩展点：增加"知识语义落地"审查维度，防"行为对但没按知识实现" |
| **逐条验证不可并行（K1,K2,K3顺序）** | 无对应 | ✅ 借鉴：控制任务顺序（知识飞轮实现方案.md §4.3） |
...
> 读完案例后，我们吸收什么、刻意不抄什么、必须自己做什么，逐项说明。落点统一对应[知识飞轮实现方案](知识飞轮实现方案.md)。

### 12.1 直接吸收（附落点）

| # | 案例机制 | 落点 | 具体改动 |
|---|---------|------|---------|
| 1 | 信息隔离：每角色独立新上下文，不共享历史会话 | [知识飞轮实现方案.md](知识飞轮实现方案.md) §2 | 明确角色每次调用独立构造上下文；历史会话只属于编排层，角色间不传递 |
| 2 | 写保护 + SHA-256 快照校验 | [知识飞轮实现方案.md](知识飞轮实现方案.md) §2/§5 | 受保护路径（知识文档/评测集/源码快照）启动前快照，运行后校验，篡改即失败 |
...
| 7 | 失败机制总账 | 知识飞轮实现方案 §4.3 薄弱点地图 | 升级为结构化 ledger：机制名+证据+失败轮次+置信度 |
...
| 项 | 案例 | 本项目 | 原因 |
|----|------|--------|------|
| 评测次数 | 仅一次 canonical 报告 | 建议 ≥5 次，报告均值与方差 | 案例加速硬件评测昂贵；本项目优先验证稳定性 |
| 退出指标 | 正确率 100% + avg_speedup | 编译必过 + 测试 confidence 达标 + holdout 验证 | 相似度只辅助归因；门限须经 PoC 校准 |
| 迭代上限 | max_loops=20 | 待 PoC 确定 | 先记录单轮信息量、耗时与收敛曲线再设上限 |
...
5. **OKF 知识格式与门禁定义**。案例 wiki 格式未公开；本项目按[知识飞轮实现方案](知识飞轮实现方案.md) §4 自定义
