...
> 来源：对 [知识飞轮案例.md](知识飞轮案例.md) §13「待澄清问题」的逐条回答（别的团队提供，2026-08-21）。
...
指 csrc/ 目录下参与目标构建的所有源文件。source_hash.py 的计算方式：递归遍历工作目录下所有文件（排除 .git/build/dist/__pycache__/.pytest_cache/*.egg-info/*.so/*.whl/*.pyc 等，排除 设计/ 目录），对每个文件的相对路径 POSIX 字符串和原始字节内容一起喂入 SHA-256。Comparator 和 Evolver 运行前后各算一次，不一致则视为越权修改。
