...
> 前置设计：本方案严格对齐 [知识飞轮流程设计.md](知识飞轮流程设计.md)（角色分工/溯源归因/反馈结构）、[门禁与评测机制.md](门禁与评测机制.md)（门禁/评测闭环）、[知识形态定义.md](知识形态定义.md)（知识形态/溯源要求）。
...
| 设计要点 | 设计文档 | 本方案实现位置 |
|---------|---------|---------------|
| 三角色分离 + 只读原则 | 知识飞轮流程设计.md §3, 门禁与评测机制.md §3.6 | coder/ review/ 各自独立，编排层只决策 |
| 知识生成 Agent 唯一执笔 | 知识飞轮流程设计.md §3.1 | generator.py 的 generate/revise |
| 溯源链接反向映射 | 知识飞轮流程设计.md §4 | knowledge/format.py + review/attributor.py |
| 两层反馈 | 知识飞轮流程设计.md §5 | review/feedback.py |
| 门禁 80% + 多信号 | 门禁与评测机制.md §2/§3.5 | gate/evaluator.py（相似度+编译+测试） |
| 用例集驱动 | 门禁与评测机制.md §3.6 | gate/evaluator.py + confidence.py |
| 防污染回滚 | 知识飞轮流程设计.md §9.5 | storage/repo.py + orchestrator 决策规则 3 |
| 重复评测 ≥5 次 | 知识飞轮流程设计.md §9.1 | storage/session.py 记录 + Phase 5 统计 |
| TDD 方案（探讨中） | 门禁与评测机制.md §7 | confidence.py 预留"通过用例数/总数"，UT 有则用 |
