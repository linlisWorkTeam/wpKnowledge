...
1. **门禁信号的替代方案**：我们的门禁是"基于知识生成的代码 vs 源码"的对比，不需要 oracle。ConSelf 证明"内部信号也能驱动改进"，支持我们用**结构/行为对比**而非测试全通过作为门禁信号（与 docs/gate.md 的多信号组合思路吻合）
...
- **自举风险直接命中 Fragility 警告**：自我改进必须有外部锚点。我们的外部锚点是源码本身（未过门禁的知识不合并 + 自动回滚，见 docs/flywheel.md §5）

## 关联

- [fragility-self-improving.md](fragility-self-improving.md) — 自我改进的脆弱性（ConSelf 的自举风险是其佐证）
- [feedback-over-form.md](feedback-over-form.md) — 反馈质量 > 流程形式
- docs/gate.md §3.5 — 门禁评测的可靠性约束
- [self-debugging.md](self-debugging.md) — 执行反馈的作用（ConSelf 的行为一致性是执行反馈的一种）
