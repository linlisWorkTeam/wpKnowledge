...
- init、ingest、query、get、status、scan、feedback 委派给 `endlessWpKnowledgeRunner/apps/runner/src/cli.ts`；
...
`endlessWpKnowledgeRunner/packages/domain` 不导入数据库、SDK、编译器或语言类型；`packages/application` 只依赖 domain 和 contracts；具体实现位于同一组件的 `packages/adapters`。`endlessWpKnowledgeRunner/tests/contract/architecture.test.ts` 自动约束这一方向。
...
完整 CAS 索引、命令输出摘要和证据边界见 [ohMyWorkPanel 固定 commit 真实源码闭环验收](../证据/2026-09-01-ohMyWorkPanel真实源码验收.md)。
...
- 规范：`endlessWpKnowledgeRunner/specs/`
- 架构：`endlessWpKnowledgeRunner/docs/ARCHITECTURE.md`
- 运维：`endlessWpKnowledgeRunner/docs/OPERATIONS.md`
- 真实场景：`endlessWpKnowledgeRunner/acceptance/ohmyworkpanel/`
- 自动化测试：`endlessWpKnowledgeRunner/tests/`
