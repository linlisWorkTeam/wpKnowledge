...
- init、ingest、query、get、status、scan、feedback 委派给 `endlessWpKnowledgeRunner/src/interfaces/runner/cli.ts`；
...
`endlessWpKnowledgeRunner/src/domain` 不导入数据库、开发工具包、编译器或语言类型；`src/application/services` 只依赖领域层和端口；具体实现位于同一组件的 `src/infrastructure`。`endlessWpKnowledgeRunner/tests/contract/architecture.test.ts` 自动约束这一方向。
