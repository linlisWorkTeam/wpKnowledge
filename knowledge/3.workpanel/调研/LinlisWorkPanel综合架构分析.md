...
| 日期快照 | [`WorkPanel 2.0.0架构评审`](./WorkPanel 2.0.0架构评审.md) |

相关中间件专题：[`WorkPanelConnecter当前实现分析`](./WorkPanelConnecter当前实现分析.md)（当前源码实现，`main@3cf0d68`）和 [`WorkPanel Connecter愿景符合度与可扩展性评审`](./WorkPanel Connecter愿景符合度与可扩展性评审.md)（历史演进评审）。前者覆盖每站 Connecter、中心 Connecter Host、Runner、Directory v2、WorkPet 和跨站消息联邦的当前数据流与证据边界。
