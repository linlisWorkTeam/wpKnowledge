...
import { translateLegacyArgs } from '../../endlessWpKnowledgeRunner/src/compat.ts';
...
test('legacy facade writes only the shared SQLite and CAS runtime', () => {
  ...
  const run = (args: string[]) => spawnSync(process.execPath, ['endlessWpKnowledgeRunner/fw.mjs', ...args], {
    cwd: process.cwd(),
